...
class Database:
    ...
    def authenticate_user(self, login: str, password: str) -> dict | str | None:
        """
        Аутентифицирует пользователя и возвращает его данные.
        
        Returns:
            dict: данные пользователя при успешной аутентификации
            str: 'inactive' если аккаунт деактивирован
            None: если неверные учётные данные
        """
        query = """
            SELECT id, login, full_name, email, role, is_active,
        ...
        try:
            ...
            if not user[5]:  # is_active = False
                return 'inactive'
            
            # Проверяем пароль
            if PasswordHelper.verify_password(password, user[8]):
    ...
    def reset_user_password(self, target_user_id: int, new_password: str, admin_user_id: int) -> dict:
        ...
        try:
            ...
            return {
                'success': True,
                'message': f"Пароль для пользователя '{user[0]}' ({user[1]}) успешно изменён"
            }
        except Exception as e:
